...
export default function CreatePoolCard() {
  ...
  const [expectedEntries, setExpectedEntries] = useState(50)
  ...
  async function ensureBase(): Promise<boolean> {
    ...
    if (!wallet?.switchChain) { toast.error('Wrong network. Please switch to Base.'); return false }
    try { await wallet.switchChain({ id: BASE_CHAIN_ID }); toast.success('Switched to Base'); return true }
    catch (e: any) { toast.error(e?.shortMessage || e?.message || 'Switch failed'); return false }
  }
  ...
  async function fetchSentinelCommit() {
    if (!env.sentinelUrl) return toast.error('Set NEXT_PUBLIC_SENTINEL_URL')
    try {
      ...
      if (!/^0x[0-9a-fA-F]{40}$/.test(addr) || !/^0x[0-9a-fA-F]{64}$/.test(hash)) throw new Error('Invalid commit payload from sentinel')
  ...
  function downloadSaltBackup(poolId: bigint, encryptedOrRaw: string) {
    try {
      const blob = new Blob([JSON.stringify({ poolId: poolId.toString(), creatorSalt: encryptedOrRaw }, null, 2)], { type: 'application/json' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a'); a.href = url; a.download = `fairplay-creator-salt-${poolId.toString()}.json`
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-3 justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className={`rounded-full px-3 py-1 text-xs ring-1 ${chainId === BASE_CHAIN_ID
      ...
      {/* BASICS */}
      <Section title="Basics" hint="Core settings for your pool.">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          />
          <LabeledInput
            label={<LabelWithTip text={`Creator bond (${symbol})`} tip="Refundable bond you stake to deter spam." />}
            value={bond} onChange={setBond} placeholder="e.g. 50.00"
          />
          <LabeledNumber
            label={<LabelWithTip text="Min entries (0 = none)" tip="Required entries for pool validity. Use 0 for none." />}
            value={minEntries} onChange={setMinEntries} min={0}
          />
          <LabeledNumber
            label={<LabelWithTip text="Max entries (0 = unlimited)" tip="Cap the pool size. Use 0 for no cap." />}
      ...
      {/* CREATOR SALT (with actions) */}
      <Section title="Creator salt" hint="Keep this safe. You’ll need it to reveal.">
        <SaltField value={creatorSalt} onChange={setCreatorSalt} />
      </Section>
