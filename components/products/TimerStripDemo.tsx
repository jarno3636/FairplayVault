...
export default function TimerStripDemo() {
  const start = Math.floor(Date.now()/1000) - 10
  const end = start + 90
  return (
    <ProductCard
      title="Timer Strip"
      subtitle="Linear progress bar until a deadline."
      demo={<TimerStrip start={start} end={end} label="Entry closes in" />}
      code={code}
    />
  )
}
