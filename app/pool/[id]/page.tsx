...
export default function PoolDetail() {
  ...
  async function refresh() {
    const p = await pub!.readContract({ address: env.vault as Address, abi, functionName: 'pools', args: [poolId] })
    setPool(p)
    const pv = await pub!.readContract({ address: env.vault as Address, abi, functionName: 'prizePreview', args: [poolId] }) as any
  ...
  async function enter() {
    ...
    setBusy(true)
    try {
      const sim = await pub!.simulateContract({ address: env.vault as Address, abi, functionName: 'enter', args: [poolId, qty], account: wallet.account! })
  ...
  async function reveal() {
    if (!wallet) return toast.error('Connect wallet')
    // Try salt under poolId (raw or encrypted)
    ...
    if (raw.startsWith('0x') === false) {
      // encrypted payload -> ask passphrase
    ...
    setBusy(true)
    try {
      const sim = await pub!.simulateContract({ address: env.vault as Address, abi, functionName: 'revealCreator', args: [poolId, salt], account: wallet.account! })
      const tx = await wallet.writeContract(sim.request)
      await pub!.waitForTransactionReceipt({ hash: tx })
  ...
  async function finalize() {
    ...
    setBusy(true)
    try {
      const sim = await pub!.simulateContract({ address: env.vault as Address, abi, functionName: 'finalizeAfterDeadlines', args: [poolId], account: wallet.account! })
      const tx = await wallet.writeContract(sim.request)
      await pub!.waitForTransactionReceipt({ hash: tx })
    ...
    <main className="space-y-4">
      <div className="card">
        <div className="flex items-center justify-between">
          <div className="text-lg font-semibold">Pool #{id}</div>
        ...
        {prizes && (<div className="mt-3 text-slate-300 text-sm">Prize {formatUsd(prizes[0])} • Builder fee {formatUsd(prizes[1])} • Protocol fee {formatUsd(prizes[2])}</div>)}
      </div>
